...
var currentCloud = cloudMapping[environment().name] ?? cloudMapping.AzureCloud
...
var aksRequiredEndpointRules = [
  ...
  }
  {
    ...
    destinationType: 'FQDN'
    #disable-next-line no-hardcoded-env-urls
    destination: 'packages.microsoft.com,acs-mirror.azureedge.net,packages.aks.azure.com,security.ubuntu.com,snapcraft.io,api.snapcraft.io,contracts.canonical.com,cdn.fwupd.org,motd.ubuntu.com'
    protocols: [
      'HTTPS'
    ]
    port: '443'
...
var hasSubnetSourceSelection = !empty(aksRequiredSourceSubnetNames)
var hasExplicitSourceCidrs = !empty(aksRequiredSourceCidrs)
var missingConnectivitySourceSentinel = 'INVALID_SOURCE_CIDR_SET_aksRequiredSourceSubnetNames_OR_aksRequiredSourceCidrs'

var aksConnectivitySourceCidr = !enableAksRequiredConnectivity
  ? ''
  : (hasSubnetSourceSelection 
  ? join(resolvedSourceCidrsFromNames, ',') 
  : (hasExplicitSourceCidrs ? aksRequiredSourceCidrs : missingConnectivitySourceSentinel))

var aksRequiredEndpointDefinitionOverrides = empty(aksRequiredEndpointDefinition) ? {} : aksRequiredEndpointDefinition
var existingAksRequiredEndpointResourceId = string(aksRequiredEndpointDefinitionOverrides.?existingResourceId ?? '')
var resolvedAksRequiredEndpointName = string(aksRequiredEndpointDefinitionOverrides.?aksCommunityEndpointName ?? aksRequiredEndpointName)
var customAksRequiredEndpointTags = aksRequiredEndpointDefinitionOverrides.?tags ?? {}
...
var customAksRequiredEndpointProperties = aksRequiredEndpointDefinitionOverrides.?properties ?? {}
...
var resolvedAksRequiredConnectionName = string(aksRequiredConnectionDefinitionOverrides.?aksEnclaveConnectionName ?? aksRequiredConnectionName)
var customAksRequiredConnectionTags = aksRequiredConnectionDefinitionOverrides.?tags ?? {}
