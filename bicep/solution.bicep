...
@description('CIDR to use when creating the GatewaySubnet if it does not exist. Default: 10.0.129.192/26')
param gatewaySubnetPrefix string = '10.0.129.192/26'
...
// Determine existing GatewaySubnet prefix (if discovered) or fall back to provided default/override
...
var effectiveGatewaySubnetPrefix = !empty(discoveredGatewaySubnetPrefix) ? discoveredGatewaySubnetPrefix : gatewaySubnetPrefix
...
// Hub naming for resources that must remain consistent/idempotent with existing hub gateway naming
module namingConventionHub '../../modules/naming-convention.bicep' = {
...
module vpnGatewayModule 'modules/vpn-gateway.bicep' = {
  ...
  params: {
    delimiter: delimiter
    location: location
  publicIpAddressName: namingConventionHub.outputs.names.virtualNetworkGatewayPublicIpAddress
  virtualNetworkGatewayName: namingConventionHub.outputs.names.virtualNetworkGateway
...
module localNetworkGatewayModule 'modules/local-network-gateway.bicep' = {
  ...
  localNetworkGatewayName: namingConventionHub.outputs.names.localNetworkGateway
...
module vpnConnectionModule 'modules/vpn-connection.bicep' = {
  ...
  scope: resourceGroup(hubResourceGroupName)
  params: {
  vpnConnectionName: '${namingConventionHub.outputs.names.virtualNetworkGateway}-to-${namingConventionHub.outputs.names.localNetworkGateway}'
    vgwlocation: location
  vpnGatewayName: namingConventionHub.outputs.names.virtualNetworkGateway
...
module createHubRoutesToOnPrem 'modules/routes.bicep' = {
  ...
  scope: resourceGroup(split(hubVirtualNetworkResourceId, '/')[2], split(hubVirtualNetworkResourceId, '/')[4])
  params: {
  // Add on-prem override routes to the dedicated VGW route table instead of the existing hub route table
  routeTableName: namingConvention.outputs.names.routeTable
